from quizz.models import Language, Theme, Question, Answer, Choice
        ...
        if not Language.objects.filter(name=lang):
            ...
            for theme in dict_word[lang]:
                ...
                for word_id in dict_word[lang][theme]['lang']:
                    print('    question')
                    question = Question()
                    question.question_text = dict_word[lang][theme]['lang'][
                        word_id]
                    question.theme = _theme
                    question.save()
                    _temp = []
                    for _word_id in dict_word[lang][theme]['mlang']:
                        _word_id_lang = dict_word[lang][theme]['lang'][
                            _word_id]
                    ...
                    for _i in range(5):
                        ...
                        if not Choice.objects.filter(choice_text=_choice,
                                                     question=question):
                            choice = Choice()
